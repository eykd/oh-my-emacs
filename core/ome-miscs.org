...
rainbow-identifiers is installed in src/ since it's not in ELPA.
...
From Amit: I first turned off font-lock colors:
...
#+BEGIN_SRC emacs-lisp
  ...
  (set-face-attribute 'font-lock-string-face nil :background "#0e0e0e")
  ;; (set-face-attribute 'font-lock-builtin-face nil :foreground nil :slant 'italic)
#+END_SRC

From Amit: I wanted a different set of colors for rainbow identifiers, so I
generated some in L*a*b* color space:
...
Recreate the *scratch* buffer on kill:
- http://www.emacswiki.org/emacs/RecreateScratchBuffer

#+NAME: recreate-scratch-buffer
#+BEGIN_SRC emacs-lisp
...
defines =M-s= as the prefix key. If you have any other
